...
def NTcrossentropy(vtx_feature, pts_feature, corr, tau=0.01):
    corr_vtx_norm = vtx_feature[corr[:, 0]]
    corr_pts_norm = pts_feature[corr[:, 1]]
    yiyj = torch.sum(corr_vtx_norm.mul(corr_pts_norm), dim=1)
    expyiyj = torch.exp(yiyj.div(tau))
    yiyk = corr_vtx_norm.matmul(pts_feature.t())
    expyiyk = torch.exp(yiyk.div(tau))
    Sig = torch.sum(expyiyk, dim=1)
    L = -torch.log(expyiyj.div(Sig))
    loss = torch.sum(L)
    return loss
...
def fit_rotation(vtx, pts, vtx_feature, pts_feature, corrmask):
    cos_sim = vtx_feature.matmul(pts_feature.t())
    maxidx = torch.max(cos_sim, dim=1)[1]
    corrmask = corrmask.ge(0.5)
    vtx = vtx[corrmask]
    maxidx = maxidx[corrmask]
    pts = pts[maxidx]
    AverageV = torch.mean(vtx, dim=0).unsqueeze_(0)
    V = vtx.sub(AverageV)
    AverageP = torch.mean(pts, dim=0).unsqueeze_(0)
    P = pts.sub(AverageP)
    S = V.t().matmul(P)
    u, d, v = S.svd()
    R = Rotation.from_matrix(u.matmul(v.t()).tolist())
...
# paths you may want to adjust, but it's better to keep the defaults    
 ...
 # hyperameters of network/options for training
    ...
    parser.add_argument("--train_corrmask", default=False, action="store_true", help="Train also the correspondence mask branch")
